...
import { Eye, X, Calendar, CheckCircle, XCircle, ChevronLeft, ChevronRight, Download, FileX } from 'lucide-react';

const API_BASE_URL = 'http://localhost:3001/presences';

const EtudiantsManagement = () => {
  const [etudiants, setEtudiants] = useState([]);
  const [showModal, setShowModal] = useState(false);
  const [modalType, setModalType] = useState(''); // 'presences' ou 'absences'
  ...
  const fetchEtudiants = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/users/roleactif/etudiant`);
  ...
  const handleViewPresences = async (etudiant) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/presences/etudiant/${etudiant.id}`);
      const data = await response.json();
      if (data.success) {
  ...
  const handleViewAbsences = async (etudiant) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/absences/etudiant/${etudiant.id}`);
      const data = await response.json();
      if (data.success) {
        setRecords(data.data || []);
      }
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-xl font-semibold">Liste des Étudiants Actifs</h3>
      </div>
                ...
                <tr key={etudiant.id} className="border-t hover:bg-gray-50 transition">
                  <td className="px-6 py-4">
                    ...
                    <div 
                      ...
                      style={{ display: etudiant.image_faciale_path ? 'none' : 'flex' }}
                    >
                  ...
                  {modalType === 'presences' ? (
                    <>
                      ...
                      <p className="text-gray-500 text-lg">
                        Cet étudiant n'a aucune présence enregistrée
                      </p>
                    </>
                  ) : (
                    <>
                      ...
                      {currentRecords.map((record, index) => (
                        <tr key={index} className="border-t hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 text-sm font-medium">
                            {record.seance?.cours?.nom || record.cours?.nom || 'N/A'}
                          </td>
...
export default EtudiantsManagement;